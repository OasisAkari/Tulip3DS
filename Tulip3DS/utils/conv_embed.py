...
def conventer(log=print, verbose=False, dev_keys=None, use_deprecated=False, game: list=None, output: str=None,
    ...
    certchain_dev = b''
    if dev_keys:
        log('Devkit keys are being used since `--dev-keys\' was passed. Note '
        ...
        log('Looking for certchain-dev.bin...')
    ...
    for arg in game:
        ...
        if len(to_add) == 0:
            ...
            for input_file in to_add:
                ...
                cia_name = os.path.join(output, rom_name + '.cia')
    ...
    if use_deprecated:
        log('Note: Deprecated options are being used. XORpads are no longer '
              'supported. See the README at https://github.com/ihaveamac/3dsconv '
              'for more details.')

    # log if pyaes is found, and search for boot9 if it is
    ...
    if pyaes_found:
        ...
        def set_keys(boot9_file):
            nonlocal keys_set, orig_ncch_key
            keys_offset = 0
        ...
        with open(rom_file[0], 'rb') as rom:
            ...
            if extheader_hash != ncch_extheader_hash:
                ...
                if ignore_bad_hashes:
                    log('Converting anyway because --ignore-bad-hashes was '
                          'passed.')
                else:
                    continue
            ...
            with open(rom_file[2], 'wb') as cia:
                ...
                log('Writing Game Executable CXI...')
                ...
                if dlpchild_cfa_offset != 0:
                    cia.seek(0, 2)
                    log('Writing Download Play child container CFA...')
