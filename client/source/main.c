...
#define tufinish_PATH "/tufinish.bin"
...
int load_tufinish(char *path, struct finish_db_entry_final **entries)
{
	FILE *fp;
	...
	if (memcmp(header.magic, "TUFINISH", 8))
	{
		printf("TUFINISH magic not found.\n");
		goto fail;
	}

	printf("TUFINISH version: %lu\n", header.version);

	if (header.version > 3)
	{
		printf("This version of Tulip3DS-Client is\n");
...
void finalize_install(void)
{
	...
	title_count = load_tufinish(tufinish_PATH, &entries);
	...
	memcpy(&ticket_buf, basetik_bin, basetik_bin_len);
...
int main(int argc, char *argv[])
{
	...
	printf("Tulip3DS-Client v0.0.1\n");

	finalize_install();
	// print this at the end in case it gets pushed off the screen
	printf("\nRepository:\n");
	printf("  https://github.com/OasisAkari/Tulip3DS\n");
